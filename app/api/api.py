...
def address_dict(addresses):
    ...
    for address in addresses:
        ...
        polygon = address.building.polygon_points
...
def unique_address_dict(address_result) -> Dict:
    for full_address, address_models in address_result.items():
        coords = [addr.coord for addr in address_models]
        # The most common polygon is most likely the correct one:
        try:
            polygons = [addr.hashable_polygon_coords for addr in address_models if addr.hashable_polygon_coords]
        ...
        address_result[full_address] = AddressResult(address=full_address,
                                                     coord=coord,
